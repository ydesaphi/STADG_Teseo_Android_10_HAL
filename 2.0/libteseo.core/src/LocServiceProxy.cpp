...
{
	GnssLocation_2_0 location;
	loc.copyToGnssLocation(location.v1_0);

    // We don't have this value, just set it to 0
    location.elapsedRealtime.flags = 0;
    location.elapsedRealtime.timestampNs = 0;
    location.elapsedRealtime.timeUncertaintyNs = 0;

	ALOGI("Report location: %s", loc.toString().c_str());
	sGnssCallback->gnssLocationCb_2_0(location);
}

void sendSatelliteListUpdate(const std::map<SatIdentifier, SatInfo>& satellites)
{
	GnssSvInfo_2_0 svInfo;
	std::vector<GnssSvInfo_2_0> svInfoList;
    ...
    auto action = [&] (auto & p) {
        p.second.copyToGnssSvInfo(&svInfo.v1_0);
        svInfo.constellation = p.second.getId().getConstellation();
        svInfoList.push_back(svInfo);

        switch(p.second.getId().getConstellation()) {
            case GnssConstellationType::GPS:
                gpsSats++;
                svInfo.constellation = GnssConstellationType::GPS;
                break;
            case GnssConstellationType::SBAS:
                sbaSats++;
                svInfo.constellation = GnssConstellationType::SBAS;
                break;
            case GnssConstellationType::GLONASS:
                gloSats++;
                svInfo.constellation = GnssConstellationType::GLONASS;
                break;
            case GnssConstellationType::QZSS:
                qzsSats++;
                svInfo.constellation = GnssConstellationType::QZSS;
                break;
            case GnssConstellationType::BEIDOU:
                beiSats++;
                svInfo.constellation = GnssConstellationType::BEIDOU;
                break;
            case GnssConstellationType::GALILEO:
                galSats++;
                svInfo.constellation = GnssConstellationType::GALILEO;
                break;
            case GnssConstellationType::IRNSS:
                irnSats++;
                svInfo.constellation = GnssConstellationType::IRNSS;
                break;
            default:
                otherSats++;
                break;
	...
	ALOGI("Send satellite list: %d satellites, %d gps, %d sbas, %d glonass, %d qzss, %d beidou, %d galileo, %d irnss, %d others",
		totalSats,
        gpsSats,
        sbaSats,
        gloSats,
        ...
        irnSats,
        otherSats);
	sGnssCallback->gnssSvStatusCb_2_0(svInfoList);
}

void sendCapabilities(uint32_t capabilities)
{
	ALOGI("Set capabilities: 0x%x", capabilities);
	sGnssCallback->gnssSetCapabilitiesCb_2_0(capabilities);
}
...
void sendMeasurements(
    const ElapsedRealtime & elapsedRealtime,
    ...
    std::vector<GnssMeasurement>& measurementsData)
{
    ...
    sGnssMeasureCallback->gnssMeasurementCb_2_0(gnssData);
}
#else
...
namespace ril {
	...
	void onUpdateNetworkState_2_0(NetworkAttributes attributes)
	{
		ALOGI("Update Network State received");
		signals.updateNetworkState_2_0(attributes);
	}
...
namespace agps
{
     ...
     * Notifies that a data connection is available and sets the name of the
     * APN, and its IP type, to be used for SUPL connections.
     * With network handle, for Gnss v2.0.
     */
	int onDataConnOpen_2_0(
        uint64_t networkHandle,
        const char* apn,
        ApnIpType apnIpType)
	{
		ALOGI("Data connection available with apn and network handle");
		(void)(networkHandle);
		(void)(apn);
		(void)(apnIpType);
		return 0;
	...
	void sendAGnssStatus(AGnssType type, AGnssStatusValue status)
	{
		ALOGI("Send AGnss status");
	    sAGnssCallback->agnssStatusCb(type, status);
	}
}
