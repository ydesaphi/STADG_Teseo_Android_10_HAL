...
void HalManager::initRilIf()
{
	...
	if(config::get().agnss.enable)
	{
		...
		rilSignals.updateNetworkState_2_0.connect(SlotFactory::create(*rilIf, &ril::Ril_If::updateNetworkState_2_0));
	}
}
#else
void HalManager::initRilIf()
...
void HalManager::initAGpsIf()
{
	if(config::get().agnss.enable)
	{
		...
		AgpsIf->ipStatusCb.connect(SlotFactory::create(LocServiceProxy::agps::sendAGnssStatus));
	}
}
#else
void HalManager::initAGpsIf()
