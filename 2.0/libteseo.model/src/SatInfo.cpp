...
void SatInfo::copyToGnssSvInfo(GnssSvInfo * dest) const
{
	...
	//for Gnss v2.0, this field no longer used
	//dest->constellation = this->id.getConstellation();
	dest->constellation = ::android::hardware::gnss::V1_0::GnssConstellationType::UNKNOWN;
...
GnssConstellationType prn2constellation(int16_t prn)
{
	...
	if(PRN_IRN_MIN <= prn && prn <= PRN_IRN_MAX) return GnssConstellationType::IRNSS;
...
int16_t prn2svid(GnssConstellationType constellation, int16_t prn)
{
	switch(constellation)
	{
		...
		case GnssConstellationType::IRNSS:
			ALOGW("prn2svid: unknown svid, returning prn as svid");
			return prn;
